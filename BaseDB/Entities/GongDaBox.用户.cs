...
namespace BaseDB
{
    ...
    public partial class 用户
    {
        ...
        public void 修改用户信息(string accountNumber, string passWord, string name, List<Guid> list_OrgId, List<Guid> list_RoleId)
        {
            修改登录用户名(accountNumber);
            修改密码(passWord);
            修改昵称(name);
            var list_OldOrg = this.所属的组织列表
               .Where(m => m.IsDelete == false && m.用户Key == Key)
               .ToList();
            ...
            else
            {
                ...
                {
                    if (!list_OldOrg.Where(m => m.组织Key == orgId).Any())
                    {
            ...
            else
            {
                ...
                {
                    if (!list_OldRole.Where(m => m.角色Key == roleId).Any())
                    {
        ...
        {
            var listRole = this.拥有的角色列表.Where(m => m.IsDelete == false && m.用户Key == Key).ToList();
            listRole.ForEach(m =>
            {
                m.删除();
            });
            var listOrg = this.所属的组织列表.Where(m => m.IsDelete == false && m.用户Key == Key).ToList();
            listOrg.ForEach(m =>
            {
                m.删除();
            });
