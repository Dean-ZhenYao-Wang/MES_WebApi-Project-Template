...
namespace Service_Domain.Services
{
    public class DomainService_Menu : IDomainService_Menu
    {
        ...
        {
            this.菜单 = unitOfWork.菜单;
            this.角色的可访问菜单 = unitOfWork.角色_菜单s;
        }

        public async Task 新增菜单_DBAsync(string name, string url, string 编号, int 序号, string iCON, Guid? 上级菜单Id)
        {
        ...
        public async Task 修改菜单_DBAsync(Guid menuId, string name, string url, string 编号, int 序号, string iCON, Guid? 上级菜单Id)
        {
            var menu = await 菜单.All().Where(m => m.Key == menuId).FirstAsync();
            menu.名称 = name;
            menu.Url = url;
            menu.编号 = 编号;
            menu.序号 = 序号;
        ...
        public async Task 删除菜单_DBAsync(Guid menuId)
        {
            var menu = await 菜单.All().Where(m => m.Key == menuId).FirstAsync();
            menu.删除();
            var role_x_menu = await 角色的可访问菜单.All().Where(m => m.IsDelete == false && m.菜单Key == menuId).ToListAsync();
